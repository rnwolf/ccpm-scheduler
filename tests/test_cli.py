...
def test_validate_ok_exit_0():
    ...
    assert r.stdout.startswith("VALID")
...
def test_usage_error_exit_2():
    ...
    assert cli("build", "only-one.csv", "two.csv", "three.csv", "four.csv").returncode == 2
...
def test_validate_json_shape(tmp_path):
    ...
    (tmp_path / "resources.csv").write_text("id,name,capacity\nr1,R,2\n")
    r = cli("validate", tmp_path / "tasks.csv", tmp_path / "resources.csv", "--json")
    assert r.returncode == 1
...
def test_build_validates_first(tmp_path):
    (tmp_path / "tasks.csv").write_text("id,name,realistic_duration,predecessor_ids,resource_ids\nA,A,4,GHOST,r1\n")
    (tmp_path / "resources.csv").write_text("id,name,capacity\nr1,R,1\n")
    r = cli(
        "build",
        tmp_path / "tasks.csv",
...
def test_build_from_json_stdin(tmp_path):
    ...
    j = json.loads(r.stdout)
    assert j["ok"]
    assert j["stats"]["critical_chain"] == ["1", "2"]
...
def test_version():
    r = cli("--version")
    assert r.returncode == 0
    assert r.stdout.startswith("ccpm-scheduler ")


def test_console_script_installed():
    r = subprocess.run(["ccpm-scheduler", "--version"], capture_output=True, text=True)
    assert r.returncode == 0
    assert r.stdout.startswith("ccpm-scheduler ")
