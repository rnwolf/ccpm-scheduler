...
def plot_schedule(
    ...
    for w in calendar or []:
        overrides[w.resource_id].append((as_int(w.start), as_int(w.end), as_int(w.capacity)))
    ...
    feed_color = {c: feed_cmap(feed_palette[i % len(feed_palette)]) for i, c in enumerate(feeding_chains)}
    ...
    if show_util:
        fig, (ax, axu) = plt.subplots(
            ...
            gridspec_kw={"height_ratios": [len(rows), max(n_res, 2)], "hspace": 0.12},
        )
    else:
    ...
    if axu is not None:
        ...
        axu.set_yticklabels([f"{r} (cap {capacity.get(r, 1)})" for r in resources], fontsize=9)
        ...
        if overrides:
            util_handles.append(Patch(facecolor="0.85", hatch="///", label="Unavailable"))
        axu.legend(
            handles=util_handles,
            loc="lower right",
            bbox_to_anchor=(1.0, 1.0),
            ncol=len(util_handles),
            fontsize=8,
            frameon=False,
        )
...
def main(
    ...
    plot_schedule(
        io.load_schedule(schedule_path),
        out_path,
        title=title,
...
if __name__ == "__main__":
    ...
    if "--resources" in argv:
        i = argv.index("--resources")
        resources_path = argv[i + 1]
        del argv[i : i + 2]
    if "--calendar" in argv:
    ...
    if "--critical-label" in argv:
        i = argv.index("--critical-label")
        critical_label = argv[i + 1]
        del argv[i : i + 2]
    if "--no-utilization" in argv:
        argv.remove("--no-utilization")
        show_util = False
    if "--no-links" in argv:
        argv.remove("--no-links")
        show_links = False
    main(
        argv[1],
        argv[2],
